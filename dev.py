...
class BskyDataCollectorApp:
        ...
        try:
            ...
            for op in commit.ops:
                if op.action == 'create' and op.path.startswith('app.bsky.feed.post/'):
                    ...
                    if post_data and self._lang_detect(post_data['text']): 
    ...
    def preprocess_text(self, text):
        ...
        # 2. Remover URLs
        text = re.sub(r'http[s]?://\S+', '', text) # URLs completas (http/https)
        text = re.sub(r'www\.\S+', '', text) # URLs que começam com www.
        text = re.sub(r'\b[a-zA-Z0-9.-]+\.[a-zA-Z]{2,6}\b(/\S*)?', '', text, flags=re.IGNORECASE) # URLs do tipo domain.tld ou domain.tld/path
        
        # 3. Converter emojis para suas descrições textuais
            # Ex: 😊 -> :rosto_sorridente_com_olhos_sorridentes:
            # Usar 'pt' para descrições em português, se disponível e desejado, ou omitir para inglês padrão.
            # A biblioteca emoji tentará o idioma, se não encontrar, usará inglês.
            # Para forçar inglês: emoji.demojize(text, language='en')
        text = emoji.demojize(text, delimiters=("_", "_"))

        # 4. Tratar hashtags: remover o '#' e manter a palavra
        text = re.sub(r'#(\w+)', r'\1', text)

        # 5. Normalizar caracteres repetidos excessivamente
            # Reduz 3 ou mais ocorrências consecutivas de qualquer caractere para apenas 2
        text = re.sub(r'(.)\1{2,}', r'\1\1', text)

        # 6. Remover espaços extras que podem ter surgido das substituições
